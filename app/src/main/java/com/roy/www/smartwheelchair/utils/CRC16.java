...
//        return new byte[] { (byte) ((0xff00 & crc) >> 8),(byte) (0xff & crc), };
    ...
    public static byte[] getCRC_IBM(byte[] data) {
        return getCRC_IBM(data, 0, data.length);
    }
     ...
     * @since 1.0
     */
    public static byte[] getCRC16_Modbus(byte[] bytes) {
        // CRC寄存器全为1
        int CRC = 0x0000ffff;
        // 多项式校验值
        ...
        Log.i("CRC16","CRC:" +result.substring(2, 4) + " " + result.substring(0, 2));
        return hexStringToByteArray(result.substring(2, 4) + result.substring(0, 2));
    }
